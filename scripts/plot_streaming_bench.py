"""Phase-4 signature figure: recurrent-state footprint (and per-step latency) vs streaming horizon.

Reads ``outputs/streaming_bench.json`` (from ``eval.streaming_bench``) and renders the two-panel
plot the thesis hangs the efficiency claim on: the transformer twin's KV-cache grows linearly with
the horizon while the Mamba state stays constant (bounded-memory streaming). Latency is shown too,
labelled with the eager-Mamba caveat (no selective-scan CUDA kernel on Windows).

    PYTHONPATH=src python scripts/plot_streaming_bench.py
"""
