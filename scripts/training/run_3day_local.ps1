...
Set-Location $root
$env:PYTHONPATH = "src"
$env:PYTORCH_CUDA_ALLOC_CONF = "expandable_segments:True"  # anti-frag for the 4GB card; the
# max_split_size_mb:64 setting still fragmented -> phase-4 mamba bs8 OOM'd in epoch 3 (2026-06-30)
...
if (-not (Test-Path $P3_DONE)) {
    ...
    for ($try = 1; $try -le 4; $try++) {
        ...
        # bs4 x grad_accum 2 = effective bs8 (matches the transformer twin's recipe); plain bs8
        # peak-OOMs the 4GB card (2026-06-30 epoch 3, 2026-07-03 first backward after resume)
