...
def fig_streaming() -> None:
    ...
    style = {
        "transformer": ("tab:red", "o", "Transformer (KV-cache)"),
    ...
    ax_mem.set(
        xscale="log",
        yscale="log",
        xlabel="horizon (token steps)",
        ylabel="recurrent state (MB)",
        title="Streaming state size vs horizon",
    )
    ...
    fig.suptitle(
        "Contribution B: bounded streaming state (100M twins, measured)", fontweight="bold"
    )
    fig.tight_layout()
...
def fig_matrix() -> None:
    ...
    ax.set(
        ylabel="recon-FID (test, seed 2026)  -- lower is better",
...
GEN = {
    "rvq_l4_512": (0.0631, 0.0661),
    "rvq_l6_512": (0.0451, 0.0356),
    "rvq_l8_512": (0.0291, 0.0309),
    "rvq_l4_1024": (0.0573, 0.0547),
    "rvq_l6_1024": (0.0405, 0.0315),
    "rvq_l8_1024": (0.0297, 0.0236),
    "fsq_g4_v512": (0.0688, 0.0587),
    "fsq_g6_v512": (0.0393, 0.0310),
    "fsq_g8_v512": (0.0287, 0.0208),
    "fsq_g4_v1024": (0.0622, 0.0525),
    "fsq_g6_v1024": (0.0367, 0.0313),
    "fsq_g8_v1024": (0.0234, 0.0207),
    "fsq_g4_v1000": (0.0550, 0.0504),
    "fsq_g6_v1000": (0.0314, 0.0291),
    "fsq_g8_v1000": (0.0281, 0.0208),
}
...
def fig_generalization() -> None:
    ...
    for stem, (tr, te) in GEN.items():
        ...
        ax.scatter(
            tr,
            te,
            c="#1f3fb0" if is_fsq else "#d9534f",
            marker="s" if is_fsq else "o",
            s=45,
            zorder=3,
        )
    lo, hi = 0.018, 0.072
    ...
    ax.text(
        0.022,
        0.063,
        "above line: test worse than train\n(overfit hint)",
        fontsize=8,
        color="tab:red",
    )
