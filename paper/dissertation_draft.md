...
$L$. Generated by `scripts/plot_streaming_bench.py` from `eval.streaming_bench`.*
...
$L=8192$** — a **225×** gap at 8192 that diverges without bound. This is guaranteed by the recurrence,
...
### 5.6 Main results — the twin table  *(transformer val locked; Mamba twin + test-set pending)*
| Model | params | FID ↓ | R@1 ↑ | Div | stream state |
|---|---|---|---|---|---|
| GT | — | — | 0.51 [HumanML3D] | 9.5 | — |
| Tokenizer ceiling (recon) | — | 0.017[*] | — | — | — |
| Transformer-100M (ours, val) | ~98M | 1.63 | 0.323 | 8.79 | $O(L)$ KV, 605 MB@8k |
| **Mamba-100M (ours)** | ~100M | *(pending)* | | | **$O(1)$, 2.68 MB** |
| 34M + AMASS (pilot, val) | 34M | 2.12[*] | 0.258[*] | 8.38[*] | — |
| T2M-GPT [T2M-GPT] (cited, test) | — | 0.116 | 0.417 | — | $O(L)$ |
| MoMask [MoMask] (cited, test, non-stream) | — | 0.045 | 0.521 | — | — |

*Ours (val): cfg $=6$, temp $1.0$, top-$p$ $0.9$, 730 val clips, 20-rep — full metrics FID $1.626$,
R@1 $0.323\pm0.011$, R@2 $0.478$, R@3 $0.580$, MM-Dist $4.428$, Div $8.793$ (run dir
`outputs/runs/evaluate/20260624T062231Z_evaluate_val`). cfg locked at 6 (beats 5 on every metric: FID $1.63$ vs $1.87$, R@1
$0.323$ vs $0.310$). Two comparability notes: (i) our rows are the **val** split (model selection);
the cited baselines are **test** — the one-time test-set evaluation (full 4384 clips, with
MultiModality) is pending and is the only table that compares to the literature directly. (ii) FID is
sample-size–biased, $\mathrm{FID}(N)\approx \mathrm{FID}(\infty)+c/N$, so our 730-clip val FID is
inflated relative to the 4384-clip test FIDs cited; the gap narrows on the test set. The claim of this
thesis is **twin parity at matched params/data/seed + bounded-state streaming (§5.5)**, not surpassing
MoMask.*
