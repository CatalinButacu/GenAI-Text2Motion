...
- **Why mamba trains ~16x slower (38 min vs 2.4 min/ep):** eager scan materializes
...
- Eval driver promoted to `text2motion.eval.evaluate` + **MultiModality** implemented (100x30,
...
- **E3 complete -- first citable generation numbers** (full test 2,189 clips, 20-rep, CFG 5.0/T1.1):
  **transformer 31.6M: FID 3.310, R@1 0.217+/-0.008, MModality 3.747 * mamba 31.8M: FID 3.928,
  R@1 0.168+/-0.006, MModality 4.059.** FID ratio **1.19x <= 1.5x** -> **gate PASSED**, E5 unblocked.
...
- Incidents (both caught by guards, both now structural): the pilot transformer ckpt failed loud on
  ...
  mid-epoch and was killed after burning 10.6 h CPU -> **`scripts/ps1/local_guard.ps1`** (heartbeat-stall
...
- **Iso-vocab Grouped-FSQ (6 groups x 512 = RVQ's vocab, matched encoder/budget, 500 ep):
...
- **E6 measured (100M twins, RTX 3050, batch 1, horizons 64->1024,
  ...
  transformer KV growing linearly 5.9->76.7 MB (28x at 1k, unbounded); CUDA peak flat 399 MB vs
  398->475 MB.** Latency, reported honestly: mamba flat ~26-33 ms/step (horizon-independent) but a
  ~2.3x higher constant than the transformer's ~12-15 ms at these horizons -- unfused per-step
  kernels x 23 layers vs fused SDPA x 12; crossover beyond the measured range; fused
...
- **Tokenizer latent-space sweep launched** (`scripts/ps1/tokenizer_sweep.ps1`, 3 variants x 500 ep,
