...
This test directly exercises :meth:`ActionPlanner.parse_completion` by constructing
...
class _ParseOnly:
    """Minimal stand-in for ActionPlanner that exposes only .parse_completion.
    ...
    def __init__(self) -> None:
        ...
        self.parse_completion = ActionPlanner.parse_completion.__get__(self, ActionPlanner)
...
class TestActionPlannerParse(unittest.TestCase):
    ...
    def test_well_formed_single_action(self):
        out = self.parser.parse_completion('[{"action": "walk forward", "until": "duration(40)"}]')
    ...
    def test_well_formed_multi_action(self):
        ...
        out = self.parser.parse_completion(completion)
    ...
    def test_trailing_garbage_is_clipped(self):
        ...
        out = self.parser.parse_completion(
    ...
    def test_invalid_json_raises(self):
        with self.assertRaises(ValueError):
            self.parser.parse_completion('not even json at all')

    def test_non_list_top_level_raises(self):
        with self.assertRaises(ValueError):
            self.parser.parse_completion('{"action": "walk", "until": "completed"}')

    def test_missing_required_keys_raises(self):
        with self.assertRaises(ValueError):
            self.parser.parse_completion('[{"action": "walk"}]')  # missing until

        with self.assertRaises(ValueError):
            self.parser.parse_completion('[{"until": "completed"}]')  # missing action
    ...
    def test_unparseable_until_raises(self):
        ...
        with self.assertRaises(ValueError):
            self.parser.parse_completion('[{"action": "walk", "until": "until reach tree"}]')

        with self.assertRaises(ValueError):
            self.parser.parse_completion('[{"action": "walk", "until": "while moving"}]')

    def test_action_dict_not_dict_raises(self):
        with self.assertRaises(ValueError):
            self.parser.parse_completion('["walk forward"]')  # string in list, not dict
