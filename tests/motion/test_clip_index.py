...
from text2motion.motion.storage import CLIP_LENGTHS_FILE, clip_frame_counts
...
def test_lengths_are_correct_and_cached(tmp_path):
    ...
    first = clip_frame_counts(vec_dir, ["a", "b"])
    ...
    assert clip_frame_counts(vec_dir, ["a", "b"]) == first
...
def test_missing_clips_are_skipped_not_invented(tmp_path):
    ...
    assert clip_frame_counts(vec_dir, ["a", "ghost"]) == {"a": 40}


def test_cache_is_invalidated_when_a_clip_changes_size(tmp_path):
    vec_dir = build(tmp_path)
    make_clip(vec_dir, "a", 40)
    assert clip_frame_counts(vec_dir, ["a"]) == {"a": 40}

    make_clip(vec_dir, "a", 120)
    assert clip_frame_counts(vec_dir, ["a"]) == {"a": 120}


def test_stale_cache_entry_is_recomputed(tmp_path):
    vec_dir = build(tmp_path)
    make_clip(vec_dir, "a", 40)
    clip_frame_counts(vec_dir, ["a"])
