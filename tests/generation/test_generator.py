...
from text2motion.generation.model import MotionTokenGenerator, token_ce_loss
...
def streamed_logits(
    gen: MotionTokenGenerator, tokens: torch.Tensor, text: torch.Tensor
) -> torch.Tensor:
    state = gen.backbone.init_state(tokens.size(0), tokens.device)
    prefix = gen.project_text_prefix(text)  # (B, P, d_model); P=1 here, stepped like stream() does
    for position in range(prefix.size(1)):
        h, state = gen.backbone.step(prefix[:, position], state)
    outs = [gen.predict_token_logits(h)]

    for t in range(tokens.size(1) - 1):
        h, state = gen.backbone.step(gen.embed_motion_tokens(tokens[:, t]), state)
        outs.append(gen.predict_token_logits(h))
...
def test_state_is_bounded_mamba_but_kv_grows_transformer():
    mamba = MotionTokenGenerator(small_cfg("mamba")).eval()
    ...
    tf = MotionTokenGenerator(small_cfg("transformer")).eval()
...
def test_single_batch_overfit_mamba():
    torch.manual_seed(0)
    cfg = small_cfg("mamba")
    gen = MotionTokenGenerator(cfg).train()
