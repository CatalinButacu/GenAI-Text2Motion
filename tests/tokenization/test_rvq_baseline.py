...
def test_overfit_one_batch():
    ...
    def step() -> float:
        ...
        loss = feature_velocity_reconstruction_loss(recon, x) + CFG.commitment_beta * commit
        opt.zero_grad()
        loss.backward()
        opt.step()
        return feature_velocity_reconstruction_loss(recon, x).item()
...
def test_codebook_updates_and_no_collapse():
    ...
    for _ in range(50):
        recon, _, commit = tok(x)
        (feature_velocity_reconstruction_loss(recon, x) + CFG.commitment_beta * commit).backward()
        opt.step()
        opt.zero_grad()
