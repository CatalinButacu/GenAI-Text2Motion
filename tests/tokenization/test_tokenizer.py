...
from text2motion.tokenization.model import FSQ, FsqTokenizer, feature_velocity_reconstruction_loss
...
def test_single_batch_overfit():
    ...
    recon0 = tok(x).recon
    loss0 = feature_velocity_reconstruction_loss(recon0, x).item()

    for _ in range(200):
        recon = tok(x).recon
        loss = feature_velocity_reconstruction_loss(recon, x)
        opt.zero_grad()
        loss.backward()
        opt.step()
