...
DEFAULT_SIZE: list[float] = [0.1, 0.1, 0.1]
...
class ObjectDefinition:
    ...
    default_size: list[float] = field(default_factory=lambda: list(DEFAULT_SIZE))
...
def load_objects() -> dict[str, ObjectDefinition]:
    ...
    return {
        name: ObjectDefinition(
            ...
            default_size=[float(x) for x in d.get("default_size", DEFAULT_SIZE)],
            default_mass=float(d.get("default_mass", DEFAULT_MASS)),
            can_be_grasped=bool(d.get("can_be_grasped", True)),
            mesh_prompt=d.get("mesh_prompt"),
