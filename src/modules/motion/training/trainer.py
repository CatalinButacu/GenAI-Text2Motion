...
def build_unified_buf(cfg, min_frames: int = 30) -> list[dict]:
    ...
    sources_key = "|".join(
        ...
        f":{getattr(getattr(cfg, name, None), 'data_dir', '')}"
        f":{getattr(getattr(cfg, name, None), 'max_samples', None)}"
        for name in ("amass", "arctic", "humanml3d", "interx")
    )
    ck = hashlib.md5(f"{sources_key}:{min_frames}".encode()).hexdigest()[:12]
    cache_path = Path("data/.cache") / f"unified_buf_{ck}.joblib"

    if cache_path.exists():
        log.info("[unified] loading buffer from cache: %s", cache_path)
        buf = joblib.load(cache_path)
        log.info("[unified] cache hit: %d samples", len(buf))

        return buf

    log.info("[unified] building buffer (first time, will cache to %s) ...", cache_path)
    ...
    log.info("[unified] cached %d processed samples -> %s", len(buf), cache_path)
...
def amass_factory(config: TrainingConfig, max_samples):
    train = MotionDataset(
        config.data_dir, "train", config.max_motion_length,
        augment=True, max_samples=max_samples,
    )
    val = MotionDataset(
        config.data_dir, "val", config.max_motion_length,
        vocab=train.vocab, max_samples=max_samples, stats=train.motion_stats,
    )
    test = MotionDataset(
        config.data_dir, "test", config.max_motion_length,
        vocab=train.vocab, max_samples=max_samples, stats=train.motion_stats,
    )
...
def humanml3d_factory(config: TrainingConfig, max_samples):
    amass_dir = getattr(config, "amass_dir", "data/AMASS")
    train = HumanML3DMotionDataset(
        data_dir=config.data_dir, amass_dir=amass_dir,
        split="train", max_motion_length=config.max_motion_length,
        max_text_length=config.max_text_length, augment=True,
    )
    val = HumanML3DMotionDataset(
        data_dir=config.data_dir, amass_dir=amass_dir,
        split="val", max_motion_length=config.max_motion_length,
        max_text_length=config.max_text_length,
        vocab=train.vocab, stats=train.motion_stats,
    )
    test = HumanML3DMotionDataset(
        data_dir=config.data_dir, amass_dir=amass_dir,
        split="test", max_motion_length=config.max_motion_length,
        max_text_length=config.max_text_length,
        vocab=train.vocab, stats=train.motion_stats,
    )
    return train, val, test, None
...
def unified_factory(config: TrainingConfig, max_samples):
    ...
    def make_src(enabled: bool, data_dir: str, amass_dir: str = "data/AMASS") -> SourceConfig:
    ...
    cfg = UnifiedConfig(
        amass=make_src("amass" in sources, getattr(config, "data_dir", "data/AMASS")),
        arctic=make_src(
            "arctic" in sources,
        ...
        interx=make_src(
            "interx" in sources,
            getattr(config, "interx_dir", "data/inter-x"),
        ),
    )
    shared_buf = build_unified_buf(cfg)
    log.info("[unified] shared buffer: %d samples  sources=%s", len(shared_buf), sources)
