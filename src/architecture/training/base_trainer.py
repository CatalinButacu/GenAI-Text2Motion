...
from src.architecture.adapters import freeze_trunk, replace_linear_with_lora
...
class BaseSSMTrainer:
    """Abstract trainer: owns state + drives the loop; subclasses implement save_checkpoint."""
    ...
    def load_frozen_tokenizer(self, config) -> MotionRVQTokenizer:
        ...
        log.info("frozen RVQ tokenizer loaded from %s", path)
    ...
    def finalize_init(self, config, train_sampler=None) -> None:
        ...
        # nn.DataParallel splits batches across CUDAs; self.model stays unwrapped for ckpt/attr.
        ...
        if n_gpus > 1 and not single_gpu:
            log.info("nn.DataParallel across %d GPUs", n_gpus)
        ...
        # Opt-in torch.compile (3-5x throughput, 30-90s first batch). Skip with DataParallel.
        if getattr(config, "compile_model", False) and self.device.type == "cuda":
            if self.n_train_gpus > 1:
                log.warning(
                    "--compile + multi-GPU DataParallel is unsupported; "
        ...
        # Adapter / LoRA mode: load base weights, freeze trunk, then build optimizer
        # over only the trainable subset. Must happen BEFORE create_optimizer below.
        adapter_mode = bool(getattr(config, "adapter_mode", False))

        if adapter_mode:
            base_ck = getattr(config, "adapter_base_ckpt", None)

            if base_ck is None or not os.path.exists(base_ck):
                raise FileNotFoundError(
                    f"adapter_mode=True requires adapter_base_ckpt to point at a "
                    f"trained checkpoint; got {base_ck!r}."
                )
            ck = torch.load(base_ck, map_location=self.device, weights_only=False)
            load_compatible(self.model, ck["model_state_dict"], "adapter_base")
            freeze_report = freeze_trunk(self.model)
            log.info(
                "[adapter] base=%s  trainable=%d/%d (%.2f%%)",
                base_ck, freeze_report["trainable"], freeze_report["total"],
                100.0 * freeze_report["trainable"] / max(freeze_report["total"], 1),
            )

            for target in getattr(config, "adapter_lora_targets", []) or []:
                replace_linear_with_lora(self.model, target,
                                         rank=int(config.adapter_lora_rank))
        ...
        if amp_on:
            log.info("mixed precision (CUDA AMP) enabled")
    ...
    def train_epoch(self, epoch: int) -> tuple:
        # self.train_model is the DP/compile-wrapped variant; underlying params live on self.model.
    ...
    def run_final_test(self) -> dict:
        if self.test_loader is None:
            log.warning("no test loader -- skipping final test evaluation")
            return {}
        metrics = run_test(
            self.train_model, self.tokenizer, self.test_loader, self.device, self.config,
        )
        log.info(
            "TEST  ce=%.4f  top1=%.3f  top5=%.3f  top10=%.3f  per_cb_acc=%.3f",
